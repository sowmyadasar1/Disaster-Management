...
export default function AdminDashboard() {
  ...
  // Quick analytics
  const [stats, setStats] = useState({ total: 0, pending: 0, inProgress: 0, resolved: 0, flagged: 0 });
  ...
  useEffect(() => {
    async function fetchReports() {
      try {
        ...
        setStats({ total: reportData.length, pending, inProgress, resolved, flagged });
  ...
  const bulkDelete = async () => {
    await Promise.all(selectedReports.map((id) => deleteDoc(doc(db, "reports", id))));
  ...
  const toggleFlag = async (id, current) => {
    ...
    setReports(reports.map(r => r.id === id ? { ...r, flagged: !current } : r));
  };
  ...
  return (
    <div className="page-container">
      ...
      {/* Quick Stats */}
      <div className="controls-container" style={{ justifyContent: "space-around" }}>
        <div><strong>Total:</strong> {stats.total}</div>
        <div><strong>Pending:</strong> {stats.pending}</div>
        <div><strong>In Progress:</strong> {stats.inProgress}</div>
        <div><strong>Resolved:</strong> {stats.resolved}</div>
        <div><strong>Flagged:</strong> {stats.flagged}</div>
      </div>

      {/* Filters & Sort */}
      <div className="controls-container" style={{ marginTop: "15px", gap: "20px" }}>
        <div>
      ...
      <div className="reports-grid">
        {filteredReports.map((report) => (
          <div key={report.id} className="report-card">
            ...
            {/* Admin Controls */}
            <div className="controls-container" style={{ marginTop: "10px", gap: "10px" }}>
